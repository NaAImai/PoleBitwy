...
const staty = [
cz = {zbroja: 0.2, przebicie: 1, szansaNaTrafienie: 1, zasiegStrzalu: 3, szanseStrzlow: [0, 0.85, 0.9]},
s = {zbroja: 0.8, przebicie: 0.4, szansaNaTrafienie: 0.75, zasiegStrzalu: 4, szanseStrzlow: [0.7, 0.8, 0.9, 0.85], walkaWZwarciu: 1},
o = {zbroja: 0.8, przebicie: 0.1, szansaNaTrafienie: 0.65, zasiegStrzalu: 3, szanseStrzlow: [0.9, 0.7, 0.45], walkaWZwarciu: 1},
b = {zbroja: 0.7, przebicie: 1, szansaNaTrafienie: 0.85, zasiegStrzalu: 3, szanseStrzlow: [1, 0.9, 0.75], walkaWZwarciu: 2},
w = {zbroja: 0.5, przebicie: 0, szansaNaTrafienie: 0.7, walkaWZwarciu: 3},
z = {zbroja: 0.5, przebicie: 0.2, szansaNaTrafienie: 0.9, zasiegStrzalu: 3, szanseStrzlow: [0.9, 0.8, 0.75], walkaWZwarciu: 3},
p = {zbroja: 0.3, przebicie: 0.2, szansaNaTrafienie: 0.95, zasiegStrzalu: 3, szanseStrzlow: [0.9, 0.75, 0.7], walkaWZwarciu: 2}
]
let templatePola = [a = [], b = [], c = [], d = [], e = [], f = [], g = [], h = []]
...
while (wygrany == 'Nikt') {
    alert(tura ? "Tura Gracza 2" : "Tura Gracza 1")
    Tura()
    if(daneJednostek[4][0] == 8 && daneJednostek[4][2] == 1){
        wygrany = 'Gracz 1 wygrał przez wysadzenie'
        break
    }
    else if(daneJednostek[11][0] == 8 && daneJednostek[11][2] == 1){
        wygrany = "Gracz 2 wygrał przez wysadzenie"
        break
    }
    for (let i = 0; i < 8; i++) {
        if(i == 7)
        {
            wygrany = "Gracz 2 wygrał przez eliminację wroga"
            break
        }
    ...
    for (let i = 7; i < 15; i++) {
        if(i == 14)
        {
            wygrany = "Gracz 1 wygrał przez eliminację wroga"
            break
        }
        else if(daneJednostek[i][2] != 0)
            break   
    ...
    if(daneJednostek.filter((value) => {return value[2] != 0}).every((value) => {return value[0] == 8}))
    {
        ...
        if(zyjacy1 > zyjacy2)
            wygrany = "Gracz 1 wygrał mając więcej jednostek"
        else if(zyjacy1 < zyjacy2)
            wygrany = "Gracz 2 wygrał przez eliminację wroga"
        else
            wygrany = "Remis - obaj gracze mają tyle samo jednostek na polu"
    }
}
...
function Wyswietlanie(poleJed, poleDwa) {
    ...
    return poleDwa.toString().replaceAll(',',"") + perzedzial + poleJed.toString().replaceAll(',',"")
}
...
function Tura()
{
    Stawianie()
    atak = ruch = true
    if(informacjaZwrotna != "") alert(informacjaZwrotna)
        informacjaZwrotna = ""
    if(tura)
    {
        while(atak || ruch)
        {    
            do 
            {
                chg = prompt(Wyswietlanie(pole2, pole1))
                if(chg == "s")
                {
                    atak = ruch = false
                    break
    ...
    else
    {
        while(atak || ruch)
            {    
                do 
                {
                    chg = prompt(Wyswietlanie(pole1, pole2))
                    if(chg == "s")
                    {
                        atak = ruch = false
                        break
...
function Ruch(jednostka) {
    if(tura)
    {
        ...
        {
            if(ktosPrzedeMna(jednostki.indexOf(jednostka) + 7) == -1 || ktosPrzedeMna(jednostki.indexOf(jednostka) + 7)[0] == -1)
            {
            ...
            {
                if(daneJednostki[0] != 7)
                {
                    daneJednostek[ktosPrzedeMna(jednostki.indexOf(jednostka) + 7)[0]][0] -= 1
                    if(ktosPrzedeMna(jednostki.indexOf(jednostka) + 7)[0] != -1) daneJednostek[ktosPrzedeMna(jednostki.indexOf(jednostka) + 7)[0]][0] -= 1
                    daneJednostki[0] += 1
                    return true
                }
                else
                {
                    daneJednostek[ktosPrzedeMna(jednostki.indexOf(jednostka) + 7)[0]][2] -= 1
                    if(ktosPrzedeMna(jednostki.indexOf(jednostka) + 7)[0] != -1) daneJednostek[ktosPrzedeMna(jednostki.indexOf(jednostka) + 7)[0]][2] -= 1
                    daneJednostki[0] += 1
                    return true
                }
            }
    ...
    else
    {
        ...
        if(daneJednostki[2] != 0 && daneJednostki[0] != 8)
        {
            if(ktosPrzedeMna(jednostki.indexOf(jednostka)) == -1 || ktosPrzedeMna(jednostki.indexOf(jednostka))[0] == -1)
            {
            ...
            else if (jednostka == "cz")
            {
                if( daneJednostki[0] != 7)
                {
                    daneJednostek[ktosPrzedeMna(jednostki.indexOf(jednostka))[0]][0] -= 1
                    if(ktosPrzedeMna(jednostki.indexOf(jednostka))[0] != -1) daneJednostek[ktosPrzedeMna(jednostki.indexOf(jednostka))[0]][0] -= 1
                    daneJednostki[0] += 1
                    return true
                }
                else
                {
                    daneJednostek[ktosPrzedeMna(jednostki.indexOf(jednostka))[0]][2] -= 1
                    if(ktosPrzedeMna(jednostki.indexOf(jednostka))[0] != -1) daneJednostek[ktosPrzedeMna(jednostki.indexOf(jednostka))[0]][2] -= 1
                    daneJednostki[0] += 1
                    return true
                }
            }
        ...
        if(daneJednostek[jednostki.indexOf(cel)][2] != 0 && daneJednostek[jednostki.indexOf(jednostka) + 7][2] != 0)
            {
                ...
                if(jednostka != "cz" && jednostka != "w" && jednostka != "b")
                {
                    ...
                    {
                        wynik = Math.random()
                        console.log(`Atak: ${wynik}, procent na trafienie: ${staty[x - 7].szanseStrzlow[Math.abs(x2 - x1) + Math.abs(y2 - y1) - 1]}`)

                        if(wynik <= staty[x - 7].szanseStrzlow[Math.abs(x2 - x1) + Math.abs(y2 - y1) - 1] || cel == "cz")
                        {
                            wynik = Math.random()
                            console.log(`Atak: ${wynik}, procent na przebicie: ${staty[y].zbroja + staty[x - 7].przebicie }`)
                            if(staty[y].zbroja + staty[x - 7].przebicie >= wynik)
                            {
                                daneJednostek[y][2] -= 1
                                informacjaZwrotna = `Wrogi: ${jednostka}, zaatakował naszego: ${cel}`
                                return true
                            }
                            else
                            {
                ...
                else if(jednostka == "cz" || jednostka == "b")
                {
                    if(jednostka == "cz")
                    {
                        ...
                        else
                        {
                            ...
                            {   
                                wynik = Math.random()
                                console.log(`Atak: ${wynik}, procent na trafienie: ${staty[x - 7].szanseStrzlow[Math.abs(x2 - x1 - 1) + Math.abs(y2 - y1) - 1]}`)

                                if(wynik <= staty[x - 7].szanseStrzlow[Math.abs(x2 - x1- 1) + Math.abs(y2 - y1) - 1] || cel == "cz")
                                {
                                    if(daneJednostek[y][2] == 1)
                                        daneJednostek[y][2] = 0
                                    else
    ...
    else
    {
        if(daneJednostek[jednostki.indexOf(cel) + 7][2] != 0 && daneJednostek[jednostki.indexOf(jednostka)][2] != 0)
        {
            ...
            if(jednostka != "cz" && jednostka != "w" && jednostka != "b")
            {
                ...
                {
                    wynik = Math.random()
                    console.log(`Atak: ${wynik}, procent na trafienie: ${staty[x].szanseStrzlow[Math.abs(x2 - x1) + Math.abs(y2 - y1) - 1]}`)
                    
                    if(wynik <= staty[x].szanseStrzlow[Math.abs(x2 - x1) + Math.abs(y2 - y1) - 1] || cel == "cz")
                    {
                        wynik = Math.random()
                        console.log(`Atak: ${wynik}, procent na przebicie: ${staty[y - 7].zbroja + staty[x].przebicie}`)
                        if(staty[y - 7].zbroja + staty[x].przebicie >= wynik)
                        {
                            daneJednostek[y][2] -= 1
                            informacjaZwrotna = `Wrogi: ${jednostka}, zaatakował naszego: ${cel}`
                            return true
            ...
            else if(jednostka == "cz" || jednostka == "b")
            {
                if(jednostka == "cz")
                {
                    ...
                    else
                    {
                        ...
                        {   
                            wynik = Math.random()
                            console.log(`Atak: ${wynik}, procent na trafienie: ${staty[x].szanseStrzlow[Math.abs(x2 - x1 - 1) + Math.abs(y2 - y1) - 1]}`)
                    
                            if(wynik <= staty[x].szanseStrzlow[Math.abs(x2 - x1 - 1) + Math.abs(y2 - y1) - 1] || cel == "cz")
                            {
                                if(daneJednostek[y][2] == 1)
                                    daneJednostek[y][2] -= 1
                                else
...
/*
    Rzeczy do zrobienia: Informacja zwrotna, balans gry. Ogólny opis.
*/